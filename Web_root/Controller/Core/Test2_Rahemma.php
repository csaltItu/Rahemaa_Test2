...
$base_dir_absolute      = "D:/xampp/htdocs/wa/";
$scripts_dir 			= $base_dir."Scripts/";
$database_dir           = $base_dir . "DBScripts/";
$prompts_dir 	        = $base_dir_absolute."prompts/";
$praat_dir 			    = $bases_dir_absolute."Praat/";
...
$test1_base  		    = "D:/xampp/htdocs/wa/RaahEMaa/";
$test1_base_absolute  	= $base_dir."RaahEMaa/";
$test1_recs  		    = $test1_base."Recs/";
$test1_prompts  		= $test1_base."Prompts/";
$test1_scripts  		= $test1_base_absolute."Scripts/";
$test1_fb_dir  			= $test1_base."Feedback/";

$polly_prompts_dir  = "";
$log_file_path            = "D:\\xampp\\htdocs\\wa\\Logs\\";
...
function ReMMenu(){
	...
	global $test1_scripts;

	sayInt($test1_prompts."Salam.wav ");
	...
	while ($loop) {
		...
		$prompt =   $test1_prompts.$options[0]['name'].$test1_prompts."1.wav ".
					$test1_prompts.$options[1]['name'].$test1_prompts."2.wav ".
					$test1_prompts."raaye.wav ".$test1_prompts."3.wav ".
					$test1_prompts."wapis.wav ".$test1_prompts."4.wav ";
...
function ReMCreateRaayeEntry(){
	...
	$result = doCurl($test1_scripts."create_fb_entry.php?uid=".$userid."&callid=".$callid);
...
function ReMRaaye(){
	...
	global $callid;
	global $userid;
	global $test1_prompts, $test1_fb_dir;
	global $test1_scripts;
	...
	while ($rerecord && $record_id) {
	    ...
	    recordAudio($test1_prompts."record-fb.wav", array(
	       "beep"=>true,
	       "timeout"=>300,
	       "bargein" => false,
	       "silenceTimeout"=>4,
	      ...
	      // "recordFormat" => "audio/wav",
	       "format" => "audio/wav",
	       "recordURI" => $test1_scripts."rem_feedback.php?record_id=".$record_id,
	        )
	    );

	    sayInt($test1_prompts."aap_nay_ye_record_karwaya_hai.wav");

	   	$filefolder = $record_id-($record_id%1000);
	   	$path       = $test1_fb_dir.$filefolder."/".$record_id.".wav";
		$path       = str_replace("\\", "/", $path);

		sayInt($path);

	    $result = gatherInput(
			$test1_prompts."agar_ye_theek_hai.wav ",
			array(
				"choices" => "[1 DIGITS]",
				"mode" => 'dtmf',
				"bargein" => true,
...
function ReMGetRecordings(){
	...
	$result = doCurl($test1_scripts."get_recordings.php");
...
function ReMSetPreference($preference, $fileid){
	...
	$result = doCurl($test1_scripts."set_preference.php?uid=".$userid."&pref=".$preference."&callid=".$callid."&fileid=".$fileid);
	...
	while ($loop) {
		...
		if ($pref_exists){
			...
			if ($newid){
				...
				/*$test1_recs_stack[$type] .= $id;
				$test1_recs_stack[$type] = substr($test1_recs_stack[$type], 1);*/
				writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", __FUNCTION__ . " Current stack: ".json_encode($test1_recs_stack[$type]));
				$test1_recs_stack[$type] = ReMRotateStack($test1_recs_stack[$type]);
				writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", __FUNCTION__ . " Rotated stack: ".json_encode($test1_recs_stack[$type]));
				$newid = false;
			}

			if ($play){
				writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", __FUNCTION__ . " ID:".json_encode($id));
				//sayInt($test1_recs.$type."-".$id.".wav");
				sayInt($test1_recs.$id[1].".wav ".$test1_recs.$id[0].".wav ");
				ReMDBLog("listen", $id[2]);
				$play = false;
			}
		}

		$prompt = $test1_prompts. 'Dobara_sunney_ke_liay.wav '.
				  //$test1_prompts. 'pasand.wav '.
				  $test1_prompts. 'more-'.$type.'.wav '.
				  //$test1_prompts. 'Aur.wav '.
				  $test1_prompts. 'Koi_aur.wav '.
				  $test1_prompts. 'Tafseelaat.wav ';
		...
		if ($result->value == "1") {
			...
			sayInt($test1_prompts."Youtube.wav ");
			$loop = true;
		}else{
			$loop = true;
		}
...
function ReMDBLog($action, $fileid){
	...
	$result = doCurl($test1_scripts."update_log.php?uid=".$userid."&action=".$action."&callid=".$callid."&fileid=".$fileid);
